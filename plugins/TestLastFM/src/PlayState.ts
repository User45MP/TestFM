...
export class PlayState {
	 ...
	 */
	private static alreadyScrobbled: boolean = false;
	...
	public static onScrobble: AddReceiver<MediaItem> = registerEmitter(async (onScrobble) => {
		...
		MediaItem.onMediaTransition(unloads, (mediaItem) => {
			...
			if (this.currentMediaItem !== undefined && !this.alreadyScrobbled) {
				...
				if (longerThan4min || moreThan50Percent) {
					onScrobble(this.currentMediaItem, this.trace.err.withContext("onScrobble"));
